...
describe("Dynamical Loading", () => {
  ...
  describe("Loading children options", () => {
    ...
    it("load children options using `loadOptions` prop", async () => {
      ...
      const spyForLoadOptions = jasmine
        .createSpy("loadOptions", loadOptions)
        .and.callThrough();
      const wrapper = mount(Treeselect, {
        propsData: {
          options: [
      ...
      expect(childrenOptionList.contains(".vue-treeselect__loading-tip")).toBe(
        true
      );
    ...
    it("handle error of loading children options & recover from it", async () => {
      ...
      expect(childrenOptionList.contains(".vue-treeselect__error-tip")).toBe(
        true
      );
      const errorText = childrenOptionList
        .find(".vue-treeselect__error-tip-text")
        .text();
    ...
    it("should override fallback nodes", async () => {
      ...
      const { vm } = wrapper;
      const getValueText = () =>
        wrapper.find(".vue-treeselect__single-value").text().trim();
    ...
    it("callback can be executed only once", () => {
      const loadOptions = jasmine
        .createSpy("loadOptions", ({ parentNode, callback }) => {
          parentNode.children = [];
          callback();
    ...
    it("load children options when doing local search", async () => {
      ...
      const labels = menu
        .findAll(
          ".vue-treeselect__option:not(.vue-treeselect__option--hide) .vue-treeselect__label"
        )
  ...
  describe("Loading root options", () => {
    ...
    it("load root options using `loadOptions` prop", async () => {
      ...
      expect(menu.firstElementChild.className).toEqual(
        jasmine.stringMatching("vue-treeselect__loading-tip")
      );
      ...
      const labels = [].slice
        .call(menu.querySelectorAll(".vue-treeselect__label"))
    ...
    it("handle error of loading root options & recover from it", async () => {
      ...
      menu = vm.getMenu();
      expect(menu.firstElementChild.className).toEqual(
        jasmine.stringMatching("vue-treeselect__error-tip")
      );
      expect(
        menu
          .querySelector(".vue-treeselect__error-tip-text")
