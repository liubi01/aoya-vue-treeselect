  ...
  describe("local search", () => {
    ...
    it("toggle expanded", async () => {
      function expectArrowToBeRotatedOrNot(expected) {
        ...
        const isRotated = optionArrow
          .classes()
          .includes("vue-treeselect__option-arrow--rotated");
    ...
    describe("matching branch nodes", () => {
      ...
      it("expand a branch node should show all its children", async () => {
        ...
        await vm.$nextTick();
        expect(wrapper.contains('.vue-treeselect__option[data-id="aa"]')).toBe(
          true
        );
        expect(wrapper.contains('.vue-treeselect__option[data-id="ab"]')).toBe(
          true
        );
        expect(wrapper.contains('.vue-treeselect__option[data-id="ac"]')).toBe(
          true
        );
        expect(vm.forest.nodeMap.ac.isExpandedOnSearch).toBe(false);
        vm.toggleExpanded(vm.forest.nodeMap.ac);
        await vm.$nextTick();
        expect(wrapper.contains('.vue-treeselect__option[data-id="aca"]')).toBe(
          true
        );
      });
    });
        ...
        const wrapper = mount(Treeselect, {
          ...
          propsData: {
            ...
            loadOptions({ action, searchQuery, callback }) {
              if (action === "ASYNC_SEARCH") {
                setTimeout(() => {
                  const option = searchQuery === "" ? "default" : searchQuery;
                  callback(null, [
                    {
                      id: option,
                      label: option,
    ...
    it("should preserve information of selected options after search query changes (old options will not be in the list)", async () => {
      ...
      const assertMultiValueItemLabels = (labels) => {
        const actualLabels = wrapper
          .findAll(".vue-treeselect__multi-value-label")
          .wrappers.map((labelWrapper) => labelWrapper.text().trim());
